...
describe("XLA RSC Waterfall tests", function () {
  ...
  it("Should add recipients correctly", async() => {
    await expect(
        xlaRSCWaterfall.connect(addr3).setRecipients([addr3.address, ], [2000, ], [BigInt(10), ], )
    ...
    await expect(xlaRSCWaterfall.setRecipients(
        [addr1.address, addr1.address],
        [ethers.utils.parseEther("100"), ethers.utils.parseEther("50")],
        [BigInt(30), BigInt(20)],
    )).to.be.revertedWithCustomError(xlaRSCWaterfall, "RecipientAlreadyAddedError")
  });
  ...
  it("Should redistribute funds correctly", async () => {
    ...
    expect((await xlaRSCWaterfall.recipientsData(addr1.address)).received).to.be.equal(0);
    ...
    expect(await xlaRSCWaterfall.currentRecipient()).to.be.equal(addr1.address);
    expect((await xlaRSCWaterfall.recipientsData(addr1.address)).received).to.be.equal(
        ethers.utils.parseEther("1")
    );
    ...
    const rscContractBalance = (await ethers.provider.getBalance(xlaRSCWaterfall.address)).toBigInt();

    expect(rscContractBalance).to.be.equal(ethers.utils.parseEther("1"))
    expect(await xlaRSCWaterfall.currentRecipient()).to.be.equal(constants.ZERO_ADDRESS);
    await expect(xlaRSCWaterfall.recipients(0)).to.revertedWithoutReason();
    ...
    const rscContractBalanceEmptyBuy = (await ethers.provider.getBalance(xlaRSCWaterfall.address)).toBigInt();
    ...
    expect(await ethers.provider.getBalance(xlaRSCWaterfall.address)).to.be.equal(0);
    expect(
    ...
    await xlaRSCWaterfall.setRecipients(
        ...
        [ethers.utils.parseEther("10"), ethers.utils.parseEther("10"), ethers.utils.parseEther("10")],
    ...
    expect((await ethers.provider.getBalance(xlaRSCWaterfall.address)).toBigInt()).to.be.equal(0);
  ...
  it("Should redistribute ERC20 token", async() => {
    ...
    await baseToken.transfer(
        xlaRSCWaterfall.address, ethers.utils.parseEther("100000")
    )

    await xlaRSCWaterfall.setTokenNativeTokenPriceFeed(baseToken.address, ethPriceFeedMock.address);
    ...
    expect(await baseToken.balanceOf(addr2.address)).to.be.equal(ethers.utils.parseEther("100000"));
    expect(await xlaRSCWaterfall.currentRecipient()).to.be.equal(addr1.address);
    expect(await xlaRSCWaterfall.numberOfRecipients()).to.be.equal(BigInt(0));

    await baseToken.transfer(
        xlaRSCWaterfall.address, ethers.utils.parseEther("1050000")
    )
    await xlaRSCWaterfall.redistributeToken(baseToken.address)
    expect(await baseToken.balanceOf(xlaRSCWaterfall.address)).to.be.equal(ethers.utils.parseEther("50000"));
    expect(await baseToken.balanceOf(addr1.address)).to.be.equal(ethers.utils.parseEther("1000000"));
    expect(await baseToken.balanceOf(addr2.address)).to.be.equal(ethers.utils.parseEther("100000"));
    ...
    await testToken.transfer(
        xlaRSCWaterfall.address, ethers.utils.parseEther("100")
    )
    await expect(
        xlaRSCWaterfall.redistributeToken(testToken.address)
    ).to.be.revertedWithCustomError(xlaRSCWaterfall, "TokenMissingNativeTokenPriceOracle");
  ...
  it("Should create manual distribution split", async() => {
    ...
    const contractBalance = (await ethers.provider.getBalance(rscWaterfallManualDistribution.address)).toBigInt();
    ...
    ).to.be.revertedWithCustomError(rscWaterfallManualDistribution, "OnlyDistributorError");
    ...
    const contractBalance2 = (await ethers.provider.getBalance(rscWaterfallManualDistribution.address)).toBigInt();
    expect(contractBalance2).to.be.equal(0);

    const investorBalanceAfter = (await ethers.provider.getBalance(addr2.address)).toBigInt();
  ...
  it("Should work with fees Correctly", async() => {
    ...
    const XLARevenueShareContract = await ethers.getContractFactory("XLARSCWaterfall");
    const xlaRSCFeeWaterfall = await XLARevenueShareContract.attach(revenueShareContractAddress);
    ...
    await baseToken.transfer(xlaRSCFeeWaterfall.address, ethers.utils.parseEther("1000000"))

    await xlaRSCFeeWaterfall.redistributeToken(baseToken.address)

    expect(await baseToken.balanceOf(platformWallet)).to.be.equal(ethers.utils.parseEther("500000"))
    expect(await baseToken.balanceOf(xlaRSCFeeWaterfall.address)).to.be.equal(ethers.utils.parseEther("150000"))
    expect(await baseToken.balanceOf(addr1.address)).to.be.equal(ethers.utils.parseEther("350000"))
  });
  ...
  it("Should work with creation ID correctly", async() => {
    ...
    await expect(xlaRSCWaterfallCreationIdFactory.createRSCWaterfall(
        {
          controller: owner.address,
          distributors: [owner.address, ],
          immutableController: false,
  ...
  it("Should recursively erc20 split", async() => {
    ...
    await baseToken.transfer(rscWaterfallMain.address, ethers.utils.parseEther("1000000"));
    await baseToken.transfer(rscWaterfallSecond.address, ethers.utils.parseEther("1000000"));
