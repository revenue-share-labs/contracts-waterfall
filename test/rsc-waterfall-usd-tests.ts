...
async function deployRSCWaterfallUsdContract(
    ...
    const xlaRSCWaterfall = await XLARSCWaterfall.attach(rscWaterfallContractAddress);
...
describe("XLA RSC Waterfall USD tests", function () {
  ...
  it("Should set base attrs correctly", async () => {
    ...
    expect(await xlaRSCWaterfallUsd.numberOfRecipients()).to.be.equal(BigInt(1));
    expect(await xlaRSCWaterfallUsd.platformFee()).to.be.equal(0);

    // Recipients settings
    expect(await xlaRSCWaterfallUsd.currentRecipient()).to.be.equal(addr2.address);
  ...
  it("Should add recipients correctly", async() => {
    await expect(
        xlaRSCWaterfallUsd.connect(addr3).setRecipients([addr3.address, ], [2000, ], [BigInt(10), ], )
    ...
    await expect(xlaRSCWaterfallUsd.recipients(2)).to.be.revertedWithoutReason();

    await expect(xlaRSCWaterfallUsd.setRecipients(
        [addr1.address, addr1.address],
        [ethers.utils.parseEther("100"), ethers.utils.parseEther("50")],
        [BigInt(30), BigInt(20)],
    )).to.be.revertedWithCustomError(xlaRSCWaterfallUsd, "RecipientAlreadyAddedError")
  });
  ...
  it("Should redistribute funds correctly", async () => {
    ...
    const addr1BalanceBefore = (await ethers.provider.getBalance(addr1.address)).toBigInt();
    const addr2BalanceBefore = (await ethers.provider.getBalance(addr2.address)).toBigInt();
    ...
    const addr1BalanceAfterFirstBuy = (await ethers.provider.getBalance(addr1.address)).toBigInt();
    const addr2BalanceAfterFirstBuy = (await ethers.provider.getBalance(addr2.address)).toBigInt();
    ...
    const addr1BalanceAfterSecondBuy = (await ethers.provider.getBalance(addr1.address)).toBigInt();
    const addr2BalanceAfterSecondBuy = (await ethers.provider.getBalance(addr2.address)).toBigInt();
    ...
    expect(await xlaRSCWaterfallUsd.currentRecipient()).to.be.equal(addr1.address);
    await expect(xlaRSCWaterfallUsd.recipients(0)).to.revertedWithoutReason();
    expect((await xlaRSCWaterfallUsd.recipientsData(addr1.address)).received).to.be.equal(0);
    ...
    const addr1BalanceAfterThirdBuy = (await ethers.provider.getBalance(addr1.address)).toBigInt();
    const addr2BalanceAfterThirdBuy = (await ethers.provider.getBalance(addr2.address)).toBigInt();

    expect(await xlaRSCWaterfallUsd.currentRecipient()).to.be.equal(addr1.address);
    expect((await xlaRSCWaterfallUsd.recipientsData(addr1.address)).received).to.be.equal(
    ...
    const addr1BalanceAfterFourthBuy = (await ethers.provider.getBalance(addr1.address)).toBigInt();
    const addr2BalanceAfterFourthBuy = (await ethers.provider.getBalance(addr2.address)).toBigInt();
    const rscContractBalance = (await ethers.provider.getBalance(xlaRSCWaterfallUsd.address)).toBigInt();

    expect(rscContractBalance).to.be.equal(ethers.utils.parseEther("1"))
    expect(await xlaRSCWaterfallUsd.currentRecipient()).to.be.equal(constants.ZERO_ADDRESS);
    await expect(xlaRSCWaterfallUsd.recipients(0)).to.revertedWithoutReason();
    ...
    const rscContractBalanceEmptyBuy = (await ethers.provider.getBalance(xlaRSCWaterfallUsd.address)).toBigInt();
    expect(rscContractBalanceEmptyBuy).to.be.equal(ethers.utils.parseEther("5"));
    ...
    const addr3BalanceBefore = (await ethers.provider.getBalance(addr3.address)).toBigInt();
    ...
    const addr3BalanceAfter = (await ethers.provider.getBalance(addr3.address)).toBigInt();

    expect(await ethers.provider.getBalance(xlaRSCWaterfallUsd.address)).to.be.equal(0);
    expect(
    ...
    const addr1BalanceBeforeSeventhdBuy = (await ethers.provider.getBalance(addr1.address)).toBigInt();
    const addr2BalanceBeforeSeventhdBuy = (await ethers.provider.getBalance(addr2.address)).toBigInt();
    const addr3BalanceBeforeSeventhdBuy = (await ethers.provider.getBalance(addr3.address)).toBigInt();
    ...
    const addr1BalanceAfterSeventhdBuy = (await ethers.provider.getBalance(addr1.address)).toBigInt();
    const addr2BalanceAfterSeventhdBuy = (await ethers.provider.getBalance(addr2.address)).toBigInt();
    const addr3BalanceAfterSeventhdBuy = (await ethers.provider.getBalance(addr3.address)).toBigInt();

    expect(await xlaRSCWaterfallUsd.numberOfRecipients()).to.be.equal(0);
    expect(await xlaRSCWaterfallUsd.currentRecipient()).to.be.equal(addr3.address);
    expect((await ethers.provider.getBalance(xlaRSCWaterfallUsd.address)).toBigInt()).to.be.equal(0);
  ...
  it("Should redistribute ERC20 token", async() => {
    ...
    await baseToken.transfer(
        xlaRSCWaterfallUsd.address, ethers.utils.parseEther("10")
    )

    await xlaRSCWaterfallUsd.setTokenUsdPriceFeed(baseToken.address, usdPriceFeedMock.address);
    await xlaRSCWaterfallUsd.redistributeToken(baseToken.address);

    expect(await baseToken.balanceOf(xlaRSCWaterfallUsd.address)).to.be.equal(0);
    expect(await baseToken.balanceOf(addr1.address)).to.be.equal(0);
    expect(await baseToken.balanceOf(addr2.address)).to.be.equal(ethers.utils.parseEther("10"));
    expect(await xlaRSCWaterfallUsd.currentRecipient()).to.be.equal(addr1.address);
    expect(await xlaRSCWaterfallUsd.numberOfRecipients()).to.be.equal(0);

    await baseToken.transfer(
        xlaRSCWaterfallUsd.address, ethers.utils.parseEther("105")
    )
    await xlaRSCWaterfallUsd.redistributeToken(baseToken.address)
    expect(await baseToken.balanceOf(xlaRSCWaterfallUsd.address)).to.be.equal(ethers.utils.parseEther("5"));
    expect(await baseToken.balanceOf(addr1.address)).to.be.equal(ethers.utils.parseEther("100"));
    expect(await baseToken.balanceOf(addr2.address)).to.be.equal(ethers.utils.parseEther("10"));
    ...
    await testToken.transfer(
        xlaRSCWaterfallUsd.address, ethers.utils.parseEther("100")
    )
    await expect(
        xlaRSCWaterfallUsd.redistributeToken(testToken.address)
    ).to.be.revertedWithCustomError(xlaRSCWaterfallUsd, "TokenMissingUsdPriceOracle");
  ...
  it("Should deploy and create immutable contract", async() => {
    ...
    await expect(rscWaterfallImmutable.setRecipients(
        [addr1.address, addr2.address, ],
        [ethers.utils.parseEther("100"), ethers.utils.parseEther("10")],
        [BigInt(10), BigInt(20)],
    )).to.be.revertedWithCustomError(rscWaterfallImmutable, "OnlyControllerError");

    await expect(rscWaterfallImmutable.connect(addr2).setController(addr2.address)).to.be.revertedWith(
        "Ownable: caller is not the owner"
    );
    await expect(
        rscWaterfallImmutable.setController(addr1.address)
    ).to.be.revertedWithCustomError(rscWaterfallImmutable, "ImmutableControllerError");
  });

  it("Should create manual distribution split", async() => {
    const rscWaterfallUsdManualDistribution = await deployRSCWaterfallUsdContract(
        owner.address,
        [owner.address, ],
        false,
        false,
    ...
    const contractBalance = (await ethers.provider.getBalance(rscWaterfallUsdManualDistribution.address)).toBigInt();
    ...
    ).to.be.revertedWithCustomError(rscWaterfallUsdManualDistribution, "OnlyDistributorError");

    const priorityRecipientBeforeBalance = (await ethers.provider.getBalance(addr2.address)).toBigInt();
    await rscWaterfallUsdManualDistribution.redistributeNativeToken()

    const contractBalance2 = (await ethers.provider.getBalance(rscWaterfallUsdManualDistribution.address)).toBigInt();
    expect(contractBalance2).to.be.equal(0);

    const priorityRecipientBalanceAfter = (await ethers.provider.getBalance(addr2.address)).toBigInt();
  ...
  it("Should work with fees Correctly", async() => {
    const RSCWaterfallFeeFactory = await ethers.getContractFactory("XLARSCWaterfallFactory");
    ...
    ).to.be.revertedWithCustomError(rscWaterfallFeeFactory, "InvalidFeePercentage");
    ...
    expect(await rscWaterfallFeeFactory.platformWallet()).to.be.equal(platformWallet);
    expect(await rscWaterfallFeeFactory.platformFee()).to.be.equal(BigInt(5000000));
    ...
    const txFee = await rscWaterfallFeeFactory.createRSCWaterfallUsd(
        {
          ...
          maxCaps: [ethers.utils.parseEther("50000"), ethers.utils.parseEther("10000")],
    ...
    const XLARevenueShareContract = await ethers.getContractFactory("XLARSCWaterfallUsd");
    const xlaRSCFeeWaterfallUsd = await XLARevenueShareContract.attach(revenueShareContractAddress);
    ...
    const addr2BalanceAfter = (await ethers.provider.getBalance(addr2.address)).toBigInt()
    const addr1BalanceAfter = (await ethers.provider.getBalance(addr1.address)).toBigInt()
    ...
    await baseToken.transfer(xlaRSCFeeWaterfallUsd.address, ethers.utils.parseEther("100"))

    await xlaRSCFeeWaterfallUsd.redistributeToken(baseToken.address)

    expect(await baseToken.balanceOf(platformWallet)).to.be.equal(ethers.utils.parseEther("50"))
    expect(await baseToken.balanceOf(xlaRSCFeeWaterfallUsd.address)).to.be.equal(ethers.utils.parseEther("15"))
    expect(await baseToken.balanceOf(addr1.address)).to.be.equal(ethers.utils.parseEther("35"))
  });

  it("Should work with creation ID correctly", async() => {
    const XLARSCWaterfallCreationIdFactory = await ethers.getContractFactory("XLARSCWaterfallFactory");
    const xlaRSCWaterfallCreationIdFactory = await XLARSCWaterfallCreationIdFactory.deploy();
    ...
    await expect(xlaRSCWaterfallCreationIdFactory.createRSCWaterfallUsd(
        {
          controller: owner.address,
          distributors: [owner.address, ],
          immutableController: false,
    ...
    )).to.be.revertedWithCustomError(xlaRSCWaterfallCreationIdFactory, "CreationIdAlreadyProcessed");
  ...
  it("Should recursively erc20 split", async() => {
    ...
    await baseToken.transfer(rscWaterfallMain.address, ethers.utils.parseEther("100"));
    await baseToken.transfer(rscWaterfallSecond.address, ethers.utils.parseEther("90"));
    ...
    expect(await baseToken.balanceOf(rscWaterfallSecond.address)).to.be.equal(0);
  });
  ...
  it("Should recursively split ETH", async() => {
    const rscWaterfallSecond = await deployRSCWaterfallUsdContract(
        ...
        ethers.utils.parseEther("1"),
        [addr1.address, addr2.address],
        [ethers.utils.parseEther("100000000"), ethers.utils.parseEther("50000000")],
        [BigInt(10), BigInt(20)],
        [baseToken.address, ],
    );
    ...
    expect(await ethers.provider.getBalance(rscWaterfallMain.address)).to.be.equal(0);
    expect(await ethers.provider.getBalance(rscWaterfallSecond.address)).to.be.equal(0);
  });
